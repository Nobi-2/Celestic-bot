...
module.exports = {
    ...
    start: async (Miku, message, { text, prefix, args, eco, ty }) => {
        ...
        if (!axeUsed) {
            return Miku.sendMessage(message.from, { text: `😕 You need to specify which axe to use (woodenAxe, woodPickaxe, stonePickaxe, ironPickaxe, platinumPickaxe).` }, { quoted: message });
        }
        ...
        if (!selectedAxe) {
            return Miku.sendMessage(message.from, { text: `😕 Invalid axe specified, valid axes are (woodenAxe, woodPickaxe, stonePickaxe, ironPickaxe, platinumPickaxe).` }, { quoted: message });
        }
